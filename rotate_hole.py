...
We should find each combination that we can rotate the backend mechanism 
so that all loaded cannonballs will be loaded into the still-working cannons. 
...
Each element describes a cannon behind the pipe; the 0th element describe 0th pipe. 
1 is a working cannon and 0 is a broken cannon.
You know the pipe numbers where the automaton will load cannonballs 
(sometimes it loads several cannonballs into one cannon).

Your goal is to find all the combinations that you can rotate the backend mechanism 
in a clockwise manner so that all of the cannonballs will be loaded into the working cannons. 
...
If you rotate the mechanism by 1 or 8 units, 
...
def rotate(state, pipe_numbers):
    result = []
    all_rotations = rot(state)
    count = 0;
    for elem in all_rotations:
        if(my_compare(elem, pipe_numbers)):
            result.append(count)
        count+= 1
    print("result:")
    print(result)
...
def rot(mylist):
    n = len(mylist)
    my_buffer = ([[mylist[i - j] for i in range(n)] for j in range(n)])
    return my_buffer

def my_compare(list1, list2):
    check = True
    for elem in list2:
        if list1[elem] == 0:
            check = False
